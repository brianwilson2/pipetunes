...
INPUT = "tunes_clean.csv"
OUTPUT = "tunes_clean_fixed.csv"

def fix_name(name):
    name = name.strip()

    # Fix "Perthshire Hills, The" -> "The Perthshire Hills"
    if name.lower().endswith(", the"):
        base = name[:-5].strip()
        return "The " + base

    # Fix "Something, A" -> "A Something"
    if name.lower().endswith(", a"):
        base = name[:-3].strip()
        return "A " + base

    # Fix "Something, An" -> "An Something"
...
print("Done. Fixed CSV saved as:", OUTPUT)
